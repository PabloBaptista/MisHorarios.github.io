...
export function initWeeekCalendar(parent,selectedDate,eventStore,isSingleDay){
    ...
    for(const weekDay of weekDays){
        ...
        //initAllDayListItem(calendarAllDayListElement,allDayEvents);   //DESCOMENTAR PARA ENCABEZADO A PRINCIPIO DEL DIA DE LOS EVENTOS FILLED
    ...
    if(!isSingleDay){
       const subEventElements= document.getElementsByClassName("sub-event--title");
       for(const subEventElement of subEventElements){ //DESCOMENTAR PARA ENCABEZADO A PRINCIPIO DEL DIA DE LOS EVENTOS FILLED
           subEventElement.innerHTML="";
       }
    }

    //Remove title, hours and span of the body of the element. Already rendered on subelements
    const dataEvents=document.getElementsByClassName("event");
    for(const dataEvent of dataEvents){
        dataEvent.getElementsByClassName("event__title")[0].innerHTML="";
        dataEvent.getElementsByClassName("event__time")[0].remove();
        dataEvent.getElementsByClassName("event__color")[0].remove(); 
    }
...
//LISTA EN EL ENCABEZADO A PRINCIPIO DEL DIA LOS EVENTOS FILLED
...
function initColumn(parent,weekDay,events){
    ...
    for(const calendarColumnCellElement of calendarColumnCellElements){
        ...
        const cellEndTime=cellStartTime+540;
        ...
        calendarColumnCellElement.addEventListener("click",()=>{
            document.dispatchEvent(new CustomEvent("event-create-request-today",{
                detail:{
                    startDate: weekDay,
                    endDate: weekDay,
                    startTravelTime: cellStartTime,
                    endTravelTime: cellEndTime
                },
                bubbles: true
            }));
        })
...
function groupEvents(events){
    ...
    for(let i=1; i<Object.keys(events).length;i+=1){
